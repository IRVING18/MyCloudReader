...
import ivring.http.utils.CheckNetwork;
 ...
 *
 * 1.4-1.7: cloudReader写完
 *   1.4:GankFragment下模块。
 *   1.5:one+book模块+准备10个左右的view以后编写总结使用
 *   1.6-1.7:总结剩余内容
 * 1.8-1.14:总结Android必要知识点
 *   1.8:表盘view编写总结，屏幕适配问题。
 *   1.9：其他适配问题，一个view编写总结。
 *   1.10:复杂view的编写。
 *   1.11：代码优化方面细节+事件分发消费机制详解+小view的总结
 *   1.12:Socket通讯问题针对装维助手
 *   1.13-1.14:一个复杂view的编写总结。其他这周总结。
 * 1.15-1.21：
 *   1.15：笔记回顾+一个小view的编写。
 *   1.16：打包，加固和混淆语法规则等+事件分发消费机制详解+一个小view的编写
 *   1.17：一个复杂view的编写+总结
 *   1.18:对基本知识点的回顾，数据库操作、Fragment的懒加载、
 *   1.19：材料设计新组件的总结、图像加载缓存机制+一个小view的编写。
 *   1.20-1.21：总结这周内容+一个复杂view的编写。
 * 1.22-1.28：
 *   1.22：Android布局优化、内存优化
 *   1.23：RxJava、RxAndroid实现响应式编程+RxBus和EventBus的概念性总结。
 *   1.24：OkHttp、Retrofit等网络访问的总结，至少要总结出一套可以答题的内容。
 *   1.25：总结一套线程间通讯（handler等），Activity和Fragment等交互的答题内容。
 *   1.26：总结一套平时遇到的难点，以及解决的方案。
 *   （1.视频测速，网页测速，下载测速好好对比一下，应该有很多值得学习的。
 *      2.基站检测也算。3.再想吧目前想不起别的特别有难度的。）
 *   1.27-1.28：一个复杂view的编写。总结这周内容，精炼一下。
 * 1.29:周一好好安排下接下来两三周做什么。
 */
